        ...
        protected override bool AddRow(DataRow row, long rowIndex, bool internalId = false)
        {
            ...
            for (int i = 0; i < colCount; i++)
            {
                ...
                if (j >= 0)
                {
                    ...
                    Variable variable = varId == 0 ? dataStructure.Variables.Where(v => v.Label.Equals(cName)).FirstOrDefault() : dataStructure.Variables.Where(v => v.Id.Equals(varId)).FirstOrDefault();
